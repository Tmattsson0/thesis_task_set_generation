...
import org.w3c.dom.Element;
    ...
    public class XMLToCSV {
        ...
        public static void main(String[] args) {
            ...
            for (int j = 1; j <= 5; j++) {
                for (int i = 0; i < 3; i++) {
                    String fileName = "testCases/var/task_set_gen_SAHC_VAR_param" + j + "_current__seed_" + i + ".xml";
        ...
        private static void writeDataToCSV(Map<String, Map<String, Double>> dataMap) {
            try (BufferedWriter writer = new BufferedWriter(new FileWriter("testCases/var/output.csv"))) {
                // Write header
                ...
                for (Map.Entry<String, Map<String, Double>> entry : dataMap.entrySet()) {
                    String fileNameWithoutPrefix = entry.getKey().replace("testCases/test1/task_set_gen_", "").replace(".xml", "");
